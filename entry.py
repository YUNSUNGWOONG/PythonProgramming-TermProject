    ...
    def admin_mode(self):
        ...
        def check_password():
            ...
            if password_entry == "admin":
                ...
                # 회원 정보를 비교하여 mixed_members 딕셔너리에 저장하는 함수
                def compare_members(members, members_time):
                    mixed_members = {}
                    for member_id, password in members.items():
                        if member_id in members_time:
                            expiration_date = members_time[member_id]
                ...
                def read_seats(file_name):
                    ...
                    with open(file_name, 'r') as file:
                        for line in file:
                            line = line.strip()
                            if line:
                                member_id, seat_number = line.split(',')
                                if member_id in seats:
                                    seats[member_id].append(seat_number)
                                else:
                                    seats[member_id] = [seat_number]
                    return seats

                # 회원 정보와 좌석 정보를 비교하여 mixed_members 딕셔너리에 저장하는 함수
                def compare_members_2(members, seats):
                    mixed_members = {}
                    for member_id, (password, expiration_date) in members.items():
                        if member_id in seats:
                            seat_numbers = seats[member_id]
                            mixed_members[member_id] = (password, expiration_date, seat_numbers)
                    return mixed_members
                ...
                # 회원 정보와 좌석 정보를 비교하여 mixed_members 딕셔너리에 저장
                ...
                # members_times_mixed.txt 파일에서 회원 정보를 읽어와 리스트로 반환하는 함수
                ...
                def display_members(members_list):
                    ...
                    for member in members_list:
                        if len(member) == 3:  # members_times_mixed.txt 정렬 시
                            line = f"ID: {member[0]}\n비밀번호: {member[1]}\n만료기한: {member[2]}\n\n"
                        else:  # members_seats.txt 정렬 시
                            line = f"ID: {member[0]}\n좌석번호: {member[1]}\n비밀번호: {member[2]}\n만료기한: {member[3]}\n\n"
                ...
                def withdraw_member():
                    ...
                    if answer:
                        ...
                        if selected_index:
                            ...
                            selected_id = members_text.get(start, end).strip()
                            if selected_id:
